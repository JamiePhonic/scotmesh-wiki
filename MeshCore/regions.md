...
5. **Support controlled interconnects**  
   Strategic repeaters can deliberately carry neighbouring traffic, such as the Ireland / Island of Ireland interconnect.
...
For users, the correct habit is:

```text
Use an agreed channel
...
The `ioi` code is not a Scottish local region. It is used for the Ireland / Island of Ireland interconnect where a repeater or message is deliberately intended to carry that traffic.
...
- `ioi` is for the Ireland / Island of Ireland interconnect.
...
### Ireland / Island of Ireland interconnect

Scottish repeaters should include the Ireland / Island of Ireland interconnect code alongside `sco`:
...
This does not make `ioi` a Scottish local region. It simply keeps the interconnect available where the Scottish setup is being configured.
...
For a repeater that serves Scotland, Central Scotland, Fife, Edinburgh, and the Ireland / Island of Ireland interconnect:
...
For local traffic, use the agreed local channel and scope. For the Ireland interconnect:
...
Yes, that is the recommended baseline for Scottish repeaters. Use `sco` for Scotland and add `ioi` alongside it for the Ireland / Island of Ireland interconnect.
...
- Add `ioi` wherever the Scottish baseline `sco` is configured.
- Use `cen`, `tay`, and `fif` for major regions.
- Use `fal`, `edi`, `gla`, `dun`, and `per` for local areas.
- Use `ioi` for the Ireland / Island of Ireland interconnect.
